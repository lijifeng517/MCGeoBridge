...
int main(int argc, char** argv) {
  if (argc < 3) {
    std::cerr << "usage: mcnp2gdml_g4check FILE.gdml gdml.xsd [overlap_resolution] [surface|interior] [overlap_tolerance_mm] [point_queries.tsv]\n";
    return 2;
  }
  ...
  if (point_result.mismatches > 0 || point_result.missing_solids > 0) return 7;
  return 0;
}
